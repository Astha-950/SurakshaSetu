...
const PostForm = ({ editPost = null, onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      organization.forEach((org) => {  console.log("tag", org);formData.append('tag', org)});
          ...
          <div className="flex flex-wrap gap-2">
            {ngos.map((org) => (
              <motion.button
                ...
                type="button"
               onClick={() => {
  if (organization.includes(org._id)) {
    setOrganization(organization.filter((o) => o !== org._id));
  } else {
    setOrganization([...organization, org._id]);
  }
}}
                className={`px-3 py-1 rounded-full text-sm transition-all ${
                  organization.includes(org._id)
